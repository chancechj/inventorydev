...
function fillData(data){
    ...
    $.each(data,function(k,v) {
        ...
        html += '<tr> ' +
            ...
            '<td>' + (v.addtime == null ? '' : getSmpFormatDateByLong(v.addtime, true)) + '</td>';
        html += '<td><a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="showDetail(\'' + v.userid + '\')">查看</a>';

        if (editrole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="edit(\'' + v.userid + '\')">修改</a>';

        if(deleterole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="del(\''+ v.userid+'\')">删除</a>';
...
function showDetail(userid){
    $.get("./"+userid,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "查看信息",
