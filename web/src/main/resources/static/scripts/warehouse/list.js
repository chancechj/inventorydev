...
function fillData(data){
    ...
    $.each(data,function(k,v) {
        var html = "";
        html += '<tr> ' +
            '<td>' + (v.storageid== null ? '' : v.storageid) + '</td>' +
            '<td>' + (v.storagename == null ? '' : v.storagename) + '</td>' +
            '<td>' + (v.storagearea == null ? '' : v.storagearea) + '</td>' ;
        html += '<td></td>><td>';
        if (editrole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="edit(\'' + v.storageid + '\')">修改</a>';
        if(deleterole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="del(\''+ v.storageid+'\')">删除</a>';
