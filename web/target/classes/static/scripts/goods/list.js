...
function fillData(data){
    ...
    $.each(data,function(k,v) {
        var html = "";
        html += '<tr> ' +
            '<td>' + (v.goodid == null ? '' : v.goodid) + '</td>' +
            '<td>' + (v.goodname == null ? '' : v.goodname) + '</td>' +
            '<td>' + (v.goodtype == null ? '' : v.goodtype) + '</td>' +
            '<td>' + (v.createdate == null ? '' : getSmpFormatDateByLong(v.createdate, true)) + '</td>';
        html += '<td><a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="showDetail(\'' + v.goodid + '\')">查看</a>';

        if (editrole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="edit(\'' + v.goodid + '\')">修改</a>';

        if(deleterole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="del(\''+ v.goodid+'\')">删除</a>';
...
function showDetail(goodid){
    $.get("./"+goodid,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "查看信息",
