...
function fillData(data){
    ...
    $.each(data,function(k,v) {
        var html = "";
        html += '<tr> ' +
            '<td>' + (v.suppliersid == null ? '' : v.suppliersid) + '</td>' +
            '<td>' + (v.suppliersname == null ? '' : v.suppliersname) + '</td>' +
            '<td>' + (v.supplierslegal == null ? '' : v.supplierslegal) + '</td>' +
            '<td>' + (v.suppliersphone == null ? '' : v.suppliersphone) + '</td>';
        html += '<td><a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="showDetail(\'' + v.suppliersid + '\')">查看</a>';

        if (editrole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="edit(\'' + v.suppliersid + '\')">修改</a>';

        if(deleterole == 'true')
            html += '<a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="del(\''+ v.suppliersid+'\')">删除</a>';
...
//分页结束
var artdialog ;
function showDetail(suppliersid){
    $.get("./"+suppliersid,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "查看信息",
